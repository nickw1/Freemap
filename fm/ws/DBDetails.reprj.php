...
class DBDetails
{
    ...
    public function getOverlayQuery($geomtxt, $type)
    {
        $tway = $this->tSRID != $this->SRID ?
            "ST_Transform(".$this->overlayDetails["col"].
            ",$this->tSRID)":
            $this->overlayDetails["col"];
        return ($this->overlayDetails) ?
            $this->trWayQuery("${type}s",
                                $this->overlayDetails["col"]) .
                " AND $tway && $geomtxt" : null;
    }
...
*/
                            ...
                            $outputFunc="ST_AsGeoJSON")
    {
        ...
        return $this->tSRID != $this->SRID ?
            ...
            "ST_Transform($geomtxt,$this->tSRID)":
            "SELECT $outputFunc(".
            "ST_Intersection($col,$geomtxt)) FROM $tbl WHERE ".
            "$col && $geomtxt";
                        ...
                        $outputFunc="ST_AsGeoJSON")
    {
        return $this->tSRID != $this->SRID ? 
            ...
            ") s) t WHERE true ":
            "SELECT $outputFunc($col) FROM $tbl WHERE true "; 
    }    
}
