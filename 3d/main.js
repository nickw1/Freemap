var Freemap3D = Class.create ({
        ...
        else
        {
			...
			*/
            ...
            var url = (layer=='osm') ?
					...
					Freemap3D.baseURL + '/0.6/ws/tsvr.php?x=' +
						tileX+ '&y=' + tileY + '&z=' + this.tileZoomLevel +
						'&poi=place,amenity,natural&way=highway&format=json'+
						'&outProj=4326':
    ...
    reloadReceived: function(xmlHTTP)
    {
        ...
        this.status('Response received OK; got ' +
            this.osmData[tileY][tileX].json.features.length + 
            ' features.');

        // add the ways to the way collection
        var str="";
        for(var count=0; count<this.osmData[tileY][tileX].json.features.length; 
            count++)
        {
            ...
            if(this.osmData[tileY][tileX].json.features[count].geometry.type==
				"LineString")
            {
                ...
                str+="Successfully added way : " +
                    this.osmData[tileY][tileX].json.features[count].
						properties.osm_id +
                    "<br/>";
                }
                catch (e)
			...
			else if (this.osmData[tileY][tileX].json.features[count].
					geometry.type=="Point")
			{
            	// only add named pois for the moment
            	if(this.osmData[tileY][tileX].json.features[count].
						properties.name)
            	{
    ...
    move: function()
    {
        ...
        {
			var p = this.camera.getForwardPosition(0.1);
			if(this.isOverground(p[0],p[1],p[2]))
			{
        ...
        {
			if(this.isOverground(this.camera.pos.x,
				this.camera.pos.y-(0.01*this.hExag),
				this.camera.pos.z))
			{
            	this.camera.up (-0.01*this.hExag);
                     this.draw();
                    this.displayPosition();
                     this.checkNewTile();
			}
	...
	getCurrentSRTM: function()
	{
		var t = this.getCurrentTile();
		return this.srtm[t[1]] ? this.srtm[t[1]][t[0]]: null;
	},

	getSurfaceHeight: function(e,n)
	{
		var srtm=this.getCurrentSRTM();
		if(srtm)
		{
			for(var i=0; i<srtm.length; i++)
			{
				var h = srtm[i].getHeight(Projection.googleToLon(e*1000.0),
							Projection.googleToLat(n*1000.0),e,n);
				if(h[0]>-1)
					return h[0];
			}
			return -1;
	...
	isOverground:function(x,y,z)
	{
		return y>=this.getSurfaceHeight(x,z);
	},
    ...
    displayPosition: function()
    {
        ...
        (90+(360-(this.camera.angle))) % 360 + " deg surf ht="+
		(this.getSurfaceHeight(this.camera.pos.x,this.camera.pos.z)*1000/
			this.hExag).toFixed(2));
    },
