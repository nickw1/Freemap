...
var OSMData = Class.create ( {
    ...
    {
        this.pois[poi.properties.osm_id] = new Object();
        for(tag in poi.properties)
        {
            if(tag!='osm_id')
                this.pois[poi.properties.osm_id][tag] = poi.properties[tag];
        }
        var e= Projection.lonToGoogle(poi.geometry.coordinates[0])/1000.0;
        var n= Projection.latToGoogle(poi.geometry.coordinates[1])/1000.0;
        this.pois[poi.properties.osm_id].x=e;
        this.pois[poi.properties.osm_id].z=n;
        this.pois[poi.properties.osm_id].y=0;
        if(srtm)
        {
        for(var srtmtile=0; srtmtile<srtm.length; srtmtile++)
        {
            var a = srtm[srtmtile].getHeight(poi.geometry.coordinates[0],
											poi.geometry.coordinates[1],e,n);
            this.pois[poi.properties.osm_id].y=a[0]+0.005*srtm[srtmtile].hExag;
        }
        }
    },
    ...
    addWay: function(way,srtm)
    {
        ...
        for(var count=0; count<way.geometry.coordinates.length; count++)
        {
            var e=Projection.lonToGoogle(way.geometry.coordinates[count][0])
					/1000.0;
            var n=Projection.latToGoogle(way.geometry.coordinates[count][1])
					/1000.0;
            ...
            for(var srtmtile=0; srtmtile<srtm.length; srtmtile++)
            {
                var a= srtm[srtmtile].getHeight
                        (way.geometry.coordinates[count][0],
                            way.geometry.coordinates[count][1],
                            e,n);
                if(a[0]>=0)
                {
                    way.vertices.push(e);
    ...
    doAllWayBuffers: function(si)
    {
		...
		{
			if(this.json.features[i].geometry.type=='LineString')
            	str+=this.doWayBuffers(this.json.features[i],si);
		}
        return str;
    },
    ...
    doWayBuffers: function(way,si)
    {
        ...
        if (!way.properties.highway)
            return;

        try
        {
            this.ways[way.properties.osm_id] = 
				new Way(way,this.wayProperties,si);
            str+="Successfully did buffer for : "  + way.properties.osm_id + 
				"<br/>";
        }
        catch (e)
        {
            str+="ERROR for way " + way.properties.osm_id+ " : " + e + "<br />";
        }
        return str;
    },
