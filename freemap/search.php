...
{
    case 'byCoord':
		$reproj=isset($cleaned['proj'])&&$cleaned['proj']=="4326";
        $x = $reproj ?  lat_to_sphmerc($cleaned["x"]): $cleaned["x"];
        $y = $reproj ?  lat_to_sphmerc($cleaned["y"]): $cleaned["y"];
    ...
    case 'byName':
        ...
        while($row=pg_fetch_array($result,null,PGSQL_ASSOC))
        {
            ...
            if($highlevel!="unknown")
            {
        		...
        		$x = $reproj ?  sphmerc_to_lon($x): $x; 
        		$y = $reproj ?  sphmerc_to_lat($y): $y;
                echo "<x>$x</x><y>$y</y>\n";
